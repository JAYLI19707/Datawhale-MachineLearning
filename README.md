...
```
.
├── tutorial.ipynb                          # 入门教程（推荐从这里开始）
├── notebooks/                              # 各阶段 Baseline 方案
│   ├── Datawhale_baseline.ipynb            # 官方 Baseline (0.63)
│   ├── Baseline_RFM.ipynb                  # RFM 用户画像建模 (0.86)
│   ├── Baseline_grid.ipynb                 # 网格搜索调优 (0.94)
│   ├── Baseline_grid_fixed.ipynb           # 修复数据泄露
│   ├── Baseline_enhanced_features.ipynb    # 深度特征工程 (0.95)
│   └── rule_bug.ipynb                      # 先验知识刷分 (0.97)
├── data/                                   # 数据文件
│   ├── train.csv                           # 训练数据（未上传，需下载）
│   ├── testA_data.csv                      # 测试数据（未上传，需下载）
│   └── dataset_infos.json                  # 数据集描述
├── images/                                 # 项目截图
│   ├── 屏幕截图 2025-07-18 174706.png
│   ├── 屏幕截图 2025-07-18 174753.png
│   └── 屏幕截图 2025-07-18 212417.png
├── README.md
├── LICENSE
└── .gitignore
```
...
**推荐从 `tutorial.ipynb` 开始**，它是一个完整的入门教程，将 6 个 Baseline 整合为一个渐进式学习路径：

| 章节 | 内容 | 你将学到 |
|------|------|----------|
| Part 1 | 数据探索 | 数据结构、类别不平衡、ID 重叠分析 |
| Part 2 | 基础特征工程 | 计数编码、目标编码、数据泄露警告 |
| Part 3 | RFM 用户画像 | 用户行为聚合、正确的特征构建方法 |
| Part 4 | 模型调优 | 网格搜索、超参数优化 |
| Part 5 | 数据泄露 | 三种泄露类型的识别与修复 |
| Part 6 | 深度特征工程 | 交叉特征、多样性特征、正则化 |
| Part 7 | 总结反思 | 经验教训与进阶方向 |
...
![项目截图1](images/屏幕截图%202025-07-18%20174706.png)

![项目截图2](images/屏幕截图%202025-07-18%20174753.png)
...
下载后将 `train.csv` 和 `testA_data.csv` 放入 `data/` 文件夹中。

| 文件 | 说明 | 大小 |
|------|------|------|
| `data/train.csv` | 训练数据 | ~283MB |
| `data/testA_data.csv` | 测试数据 | ~92MB |
| `data/dataset_infos.json` | 数据集描述 | - |

**注意**：由于数据文件较大，超过了 GitHub 的文件大小限制，因此未包含在此仓库中。请通过上述链接下载。
